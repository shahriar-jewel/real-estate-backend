...
export class Controller {
     ...
     */
    public onGet(route: string, callback: ActionCallback, roles: Role[] = []) {
        this.registerAction(HttpMethods.GET, route, callback, roles);
    }

    public onPost(route: string, callback: ActionCallback, roles: Role[] = []) {
        this.registerAction(HttpMethods.POST, route, callback, roles);
    }

    public onPut(route: string, callback: ActionCallback, roles: Role[] = []) {
        this.registerAction(HttpMethods.PUT, route, callback, roles);
    }

    public onPatch(route: string, callback: ActionCallback, roles: Role[] = []) {
        this.registerAction(HttpMethods.PATCH, route, callback, roles);
    }

    public onDelete(route: string, callback: ActionCallback, roles: Role[] = []) {
        this.registerAction(HttpMethods.DELETE, route, callback, roles);
    }

    public registerAction(method: HttpMethods, route: PathParam, callback: ActionCallback, roles: Role[] = []): void {
